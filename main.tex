...
\begin{tikzpicture}[scale=5, domain=-0.1:1.1]
	\newcommand{\offset}{0.1}
	\newcommand{\xmin}{-\offset}
	\newcommand{\ymin}{-\offset}
	\newcommand{\xmax}{1.1}
	\newcommand{\ymax}{1}

	% Axes
	\draw[black, thick, ->] (0, \ymin) -- (0, \ymax + \offset) node[above] {$x_2$};
	\draw[black, thick, ->] (\xmin, 0) -- (\xmax + \offset, 0) node[right] {$x_1$};

	% Rectangle
	\newcommand{\recty}{0.75}
	\newcommand{\rectx}{1}
	\filldraw[draw=black!75, fill=gray!50, thick] (0, 0) rectangle (\rectx, \recty);

	% Boundary
	\draw[black, thick, variable=\x, domain=\xmin:\recty+\offset] plot ({\x}, {0.75 - \x});

	% Colors
	\coordinate (A) at (\xmin, \recty + \offset);  % Top left, line r
	\coordinate (B) at (\recty + \offset, \ymin);  % Bottom right, line r
	\fill[opacity=0.25, color=orange] (\xmin, \ymin) -- (A) -- (B) -- cycle;
	\fill[opacity=0.25, color=blue] (A) -- (\rectx + \offset, \recty + \offset) -- (\rectx + \offset, \ymin) --  (B) -- cycle;

	% Point
	\newcommand{\Xx}{0.8}
	\newcommand{\Xy}{0.5}
	\filldraw[black!80] (\Xx, \Xy) circle (0.5pt) node[above] {$\vb{X}$};

	% Dashed lines
	\draw[black!75, dashed, <->] (\recty - \Xy, \Xy) -- (\rectx, \Xy);
\end{tikzpicture}
