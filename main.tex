...
\begin{tikzpicture}
	...
	\matrix[layer, label=$L_1$, right=of input] (hidden 1) {
		$x_1^1$ \\
		$x_2^1$ \\
		$x_3^1$ \\
		$x_4^1$ \\
	...
	\matrix[layer, label=$L_2$, right=of hidden 1] (hidden 2) {
		$x_1^2$ \\
		$x_2^2$ \\
		$x_3^2$ \\
		$x_4^2$ \\
	...
	\matrix[layer, label=$L_3$, right=of hidden 2] (output) {
		$o_1$ \\
		$o_2$ \\
		$o_3$ \\
	};
