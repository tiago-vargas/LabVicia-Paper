...
\begin{align*}
	x_1^{(1)} &= \sigma(w_{1,1} x_1^{(0)} + w_{1,2} x_2^{(0)} + w_{1,3} x_3^{(0)} + w_{1,4} x_4^{(0)} + b_1^{(1)}) \\
	&= \sigma(\sum_{i=1}^{4}{w_{1,i} x_i^{(1)}} + b_1^{(1)}) \\
	\vb{x}^{(1)} &= \sigma(\vb{W}\vb{x}^{(0)} + \vb{b}^{(1)})
\end{align*}

\begin{align*}
	x_1^{(1)} &= \sigma(w_{1,1} x_1^{(0)} + w_{1,2} x_2^{(0)} + \dots + w_{1,n} x_n^{(0)} + b_1) \\
	\vb{x}^{(1)} &= \sigma(\sum_{i=1}^{n}{w_{1,i} x_i^{(1)}} + b_1) \\
	       &= \sigma(\vb{W}\vb{x}^{(0)} + \vb{b})
\end{align*}

We denote $\vb{x}^{(0)}$ as $\vb{x}$, and $\vb{x}^{(n)}$ as $\vb{o}$.
