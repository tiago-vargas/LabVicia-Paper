...
\tikzset{
	layer/.style={matrix of nodes, row sep=0.5cm, nodes=neuron},
...
\begin{tikzpicture}
	...
	{
		% First layer
		\node[neuron] (x11) {$x_1^1$} & \node[neuron] (x12) {$x_1^2$}; \\
		\node[neuron] (x21) {$x_2^1$} & \node[neuron] (x22) {$x_2^2$}; \\
		\node[neuron] (x31) {$x_3^1$} & \node[neuron] (x32) {$x_3^2$}; \\
		\node[neuron] (x41) {$x_4^1$} & \node[neuron] (x42) {$x_4^2$}; \\
		\node[neuron] (x51) {$x_5^1$}; \\
	};
\end{tikzpicture}
