...
\newcommand{\drawAxes}{
	\draw[axis] (0, \ymin) -- (0, \ymax + \margin) node[above] {$x_2$};
	\draw[axis] (\xmin, 0) -- (\xmax + \margin, 0) node[right] {$x_1$};
}
...
\begin{tikzpicture}[scale=5, domain=-0.1:1.1]
	...
	% % Dashed lines
	% \draw[black!75, dashed, <->] (\rectHeight - \Xy, \Xy) -- (\rectLength, \Xy);
	\coordinate (XRect) at \X -| (\rectLength, 0);
	\coordinate (LineX) at \X -| (0, 0);
\end{tikzpicture}
